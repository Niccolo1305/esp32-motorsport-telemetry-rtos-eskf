...
#include <SD.h>
#include <SPI.h>
#include <TinyGPSPlus.h> // GPS NMEA parser — install from Library Manager: "TinyGPSPlus" by Mikal Hart
#include "eskf.h"        // Planar ESKF 2D — IMU+GPS fusion (v0.8.0)
#include <WiFi.h>
#include <algorithm>
#include <atomic>     // std::atomic<int/bool> lock-free thread safety
#include <esp_wifi.h> // esp_wifi_stop/deinit/init/start for RAM deallocation
#include <math.h>
#include <stdio.h>
#include <string.h>
...
// ── WiFi / MQTT runtime configuration ────────────────────────────────────────
// Credentials are never hardcoded. Loaded from /wifi_config.txt on SD at boot.
// If the file is absent or contains no valid SSID, WiFi is silently disabled.
//
// File format (/wifi_config.txt on SD card root):
//   SSID=MyNetwork
//   PASSWORD=mypassword
//   SSID2=FallbackNetwork      (optional, second network)
//   PASSWORD2=fallback_pass    (optional)
...
static WifiCredential wifi_networks[MAX_WIFI_NETWORKS];
static int wifi_network_count = 0;

static char cfg_mqtt_broker[64]    = "";
static int  cfg_mqtt_port          = 1883;
static char cfg_mqtt_topic[128]    = "";
static char cfg_mqtt_client_id[32] = "";
static bool mqtt_enabled           = false; // true only if MQTT_BROKER is configured

const float FREQ_HZ = 50.0f;
const float DT = 1.0f / FREQ_HZ;
const int DT_MS = (int)(DT * 1000.0f); // 20ms

// ── Statistical ZUPT/ZARU Engine — constants ──────────────────────────────
static constexpr int VAR_BUF_SIZE = 50;          // 1 s of samples at 50 Hz
static constexpr float VAR_STILLNESS_THRESHOLD = 0.05f; // [°/s]² ema_gz variance threshold
...
// AtomS3 Grove port pins for the GPS module
#define GPS_RX_PIN                                                             \
  1 // Grove: module TX → MCU RX (confirmed by hardware test)
#define GPS_TX_PIN                                                             \
  2 // Grove: module RX → MCU TX (confirmed by hardware test)
#define GPS_BAUD                                                               \
  115200 // ATGM336H-6N ships at 115200 (non-standard, not 9600)

// ==========================================================
// --- BLOCK 1.6: MICRO SD — PINS AND STRUCTURES ---
// ==========================================================
...
// Binary header written at the start of every .bin file on the SD (v0.9.7).
...
struct __attribute__((packed)) FileHeader {
  ...
  char firmware_version[16]; // e.g. "v0.9.12\0" + padding
...
QueueHandle_t sd_queue =
    NULL; // NULL until SD is mounted: guard in Task_Filter
char current_log_filename[32] = "";

// ==========================================================
// --- BLOCK 1.5: ELLIPSOIDAL CALIBRATION (Tumble Test) ---
// ==========================================================
//
...
// ==========================================================
// --- BLOCK 2: FREERTOS & GLOBAL VARIABLES ---
// ==========================================================
...
// ── GPS ──────────────────────────────────────────────────────────────────────
// TinyGPSPlus: NMEA parser. Receives chars from gpsSerial and updates internal
// fields (location, speed, satellites) on every valid decoded sentence.
// HardwareSerial(1) = ESP32-S3 UART1, mapped to Grove pins RX=1, TX=2.
TinyGPSPlus gps;
HardwareSerial gpsSerial(1);

// GPS data shared between Task_GPS (writer) and loop()/Task_Filter (readers).
...
// ── ESKF 2D ─────────────────────────────────────────────────────────────────
// Planar 2D Kalman filter instance. Lives entirely in Task_Filter (Core 1):
// no mutex needed — single-thread access.
ESKF2D eskf;

// ── ESKF 6D Shadow Mode (v0.9.8) ────────────────────────────────────────────
// State-augmented filter with gyro bias. Runs in parallel with the 5D on the
// same data for A/B testing. SD log only — not in display/MQTT.
ESKF_6D eskf6;

// ENU origin: first valid GPS coordinate after boot.
// All subsequent coordinates are converted to metres relative to this point.
double gps_origin_lat = 0.0;
double gps_origin_lon = 0.0;
bool gps_origin_set = false;
// ─────────────────────────────────────────────────────────────────────────────

// wifi_enabled: written by loop() (triple click), read by connect_wifi()
// and reconnect_network() on Core 1. atomic<bool> guarantees cross-core visibility.
...
bool sd_mounted = false;   // true if SD mounted successfully in setup()
bool sd_low_space = false;  // true if free space < 50 MB at boot
std::atomic<bool> sd_write_error{false};  // true if SD write fails at runtime
std::atomic<uint32_t> sd_records_written{0}; // records-written counter (for MQTT heartbeat)
std::atomic<uint32_t> gps_uart_overflow_count{0}; // GPS UART overflow (ISR callback)
std::atomic<bool> gps_stale{false}; // true if last GPS fix > 5 s ago (v0.9.11)
...
// Display sleep (single-click toggle with confirmation)
bool display_off = false;         // true when LCD is sleeping
bool display_confirm_pending = false; // true during 5 s confirmation window
int display_confirm_cycles = 0;   // countdown cycles for confirmation

// system_state: written by loop(), read by Task_Filter (Core 1).
// atomic<int> guarantees C++11 correctness: atomic 32-bit load/store on
// ESP32-S3. prev_system_state: used only in loop() → no atomic needed.
std::atomic<int> system_state{0};
int prev_system_state = -1;
int countdown_cycles = 0;

// buf 256 bytes: JSON payload with GPS worst-case ~210 chars
// {"ax":-9.99,"ay":-9.99,"az":-9.99,"gx":-2000.00,"gy":-2000.00,"gz":-2000.00,
//  "lap":1,"lat":45.123456,"lon":9.123456,"spd":200.0,"sats":12}
// buf 384 bytes: JSON payload with GPS + ESKF worst-case ~320 chars
char buf[384];

// ==========================================================
// --- BLOCK 3: MATH UTILITIES ---
// ==========================================================
...
void rotate_3d(float x, float y, float z, float &x2, float &y1, float &z2,
...
float trimmed_mean(float *arr, int n) {
  std::sort(arr, arr + n);
  int trim = (int)(n * 0.1f);
  float sum = 0.0f;
  int count = 0;
...
void setLabel(int y, const char *text, uint16_t txtColor = WHITE,
...
// ==========================================================
// --- BLOCK 7: NETWORK AND WIFI MANAGEMENT ---
// ==========================================================
...
// Task_GPS: Core 0, priority 2 (below Task_I2C at priority 3).
...
// ==========================================================
// --- BLOCK 7.5: TASK SD WRITER ---
// ==========================================================

// Task_SD_Writer: Core 1, priority 1 (background).
...
// ==========================================================
// --- BLOCK 8: FREERTOS TASKS ---
// ==========================================================

// Task_I2C: Core 0, priority 3.
...
void setup() {
  ...
  if (sd_mounted) {
    ...
    if (newFile) {
      // Write FileHeader (25 bytes) as the first block of the .bin file.
...
// ==========================================================
// --- BLOCK 10: MAIN LOOP (Core 1, Arduino task) ---
// ==========================================================
